  ...
  let text = m.message.buttonsResponseMessage?.selectedDisplayText || m.message.templateButtonReplyMessage?.selectedDisplayText || m.message.listResponseMessage?.title
    let isIdMessage = false, usedPrefix
        for (let name in global.plugins) {
            let danzy = global.plugins[name]
            if (!danzy) continue
            if (danzy.disabled) continue
            if (!opts['restrict']) if (danzy.tags && danzy.tags.includes('admin')) continue
            if (typeof danzy !== 'function') continue
            if (!danzy.command) continue
            const str2Regex = str => str.replace(/[|\\{}()[\]^$+*?.]/g, '\\$&')
            let _prefix = danzy.customPrefix ? danzy.customPrefix : this.prefix ? this.prefix : global.prefix
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                let isId = danzy.command instanceof RegExp ? // RegExp Mode?
                    ...
                    Array.isArray(danzy.command) ? // Array?
                        ...
                        typeof danzy.command === 'string' ? // String?
                            ...
                            false
                if (!isId) continue
                console.log({ name, command: danzy.command, text: id })
                isIdMessage = true
            }
